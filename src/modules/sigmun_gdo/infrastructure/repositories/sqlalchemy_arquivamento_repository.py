...
class SQLAlchemyArquivamentoRepository(RepositorioArquivamento):
    ...
    def get_by_id(self, id: str) -> ArquivamentoDocumento | None:
        model = (
            self._session.query(ArquivamentoDocumentoModel)
            .filter(ArquivamentoDocumentoModel.id == uuid.UUID(id))
            .first()
        )
        if not model:
            return None
    ...
    def find_by_documento(self, documento_id: str) -> list[ArquivamentoDocumento]:
        models = (
            self._session.query(ArquivamentoDocumentoModel)
            .filter(ArquivamentoDocumentoModel.documento_id == uuid.UUID(documento_id))
            .order_by(ArquivamentoDocumentoModel.data_arquivamento.desc())
            .all()
        )
