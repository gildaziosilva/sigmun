...
from src.modules.sigmun_gdo.domain.entities import (
    Documento, StatusDocumento, TipoTramitacao,
    ClassificacaoDocumental, TramitacaoDocumento, ProcessoDocumento,
    TabelaTemporalidade, TipoDestinacao,
)
from src.modules.sigmun_gdo.domain.exceptions import (
    CodigoDocumentalDuplicadoError, DocumentoNaoEncontradoError,
    IntegridadeInvalidaError,
)
from src.modules.sigmun_gdo.application.interfaces import (
    RepositorioDocumento, RepositorioTramitacao, RepositorioClassificacaoDocumental,
    RepositorioTipoDocumental,
)
from src.modules.sigmun_gdo.application.use_cases import (
    CriarDocumentoUseCase, ClassificarDocumentoUseCase, TramitarDocumentoUseCase,
)
...
from src.modules.sigmun_gdo.domain.value_objects import (
    CodigoDocumental, HashIntegridade, NumeroDocumento,
)
from src.modules.sigmun_gdo.application.use_cases.criar_documento_use_case import CriarDocumentoInputDTO
from src.modules.sigmun_gdo.application.use_cases.classificar_documento_use_case import ClassificarDocumentoInputDTO
...
class _FakeBase:
    ...
    def save(self, entity):
        ...
        return entity
    def delete(self, entity_id): return self._data.pop(entity_id, None) is not None

class FakeDocumentoRepository(_FakeBase, RepositorioDocumento):
    def get_by_codigo(self, codigo, ano):
        return next((d for d in self._data.values() if d.codigo == codigo and d.ano == ano and not d.is_deleted), None)
...
class FakeTipoDocumentalRepository(_FakeBase, RepositorioTipoDocumental):
    ...
    def get_by_codigo(self, codigo):
        return next((t for t in self._data.values() if t.is_ativo), None) if self._data else type("Tipo", (), {"id": "default", "codigo": codigo, "nome": "Default", "is_ativo": True})()
...
class TestCriarDocumentoUseCase:
    def test_cria_documento_com_sucesso(self):
        ...
        dto = CriarDocumentoInputDTO(codigo="DOC-001", numero="001", ano=2026,
            tipo_documental_id="tipo-001", titulo="Teste", descricao="Doc teste",
            unidade_autor_id="und-01", created_by="user-1")
    ...
    def test_codigo_duplicado_levanta_erro(self):
        repo = FakeDocumentoRepository()
        doc1 = Documento(codigo="DOC-001", numero="001", ano=2026, titulo="T1",
            tipo_documental_id="t1", unidade_autor_id="u1", created_by="user-1")
        repo.save(doc1)
        repo_tipo = FakeTipoDocumentalRepository()
        uc = CriarDocumentoUseCase(repo, repo_tipo)
        dto = CriarDocumentoInputDTO(codigo="DOC-001", numero="002", ano=2026,
            tipo_documental_id="tipo-002", titulo="Duplicado", descricao="Doc duplicado",
            unidade_autor_id="und-02", created_by="user-2")
        with pytest.raises(CodigoDocumentalDuplicadoError): uc.execute(dto)

    def test_hash_invalido_levanta_erro(self):
        repo = FakeDocumentoRepository()
        repo_tipo = FakeTipoDocumentalRepository()
        uc = CriarDocumentoUseCase(repo, repo_tipo)
        dto = CriarDocumentoInputDTO(codigo="DOC-002", numero="002", ano=2026,
            tipo_documental_id="t1", titulo="Hash inv", descricao="Doc hash invalido",
            unidade_autor_id="u1", hash_integridade="123", created_by="user-1")
        with pytest.raises(IntegridadeInvalidaError): uc.execute(dto)

    def test_documento_com_conteudo_ref(self):
        repo = FakeDocumentoRepository()
        repo_tipo = FakeTipoDocumentalRepository()
        uc = CriarDocumentoUseCase(repo, repo_tipo)
        dto = CriarDocumentoInputDTO(codigo="DOC-003", numero="003", ano=2026,
            tipo_documental_id="t1", titulo="Com ref", descricao="Doc com referencia",
            unidade_autor_id="u1", conteudo_ref="/files/doc003.pdf",
    ...
    def test_documento_sem_processo(self):
        ...
        repo_tipo = FakeTipoDocumentalRepository()
        uc = CriarDocumentoUseCase(repo, repo_tipo)
        dto = CriarDocumentoInputDTO(codigo="DOC-004", numero="004", ano=2026,
            tipo_documental_id="t1", titulo="Sem proc", descricao="Doc sem processo",
            unidade_autor_id="u1", created_by="user-1")
        result = uc.execute(dto)
...
class TestClassificarDocumentoUseCase:
    def _setup(self):
        ...
        doc = Documento(codigo="D1", numero="1", ano=2026, titulo="T", tipo_documental_id="t1",
            unidade_autor_id="u1", created_by="u1")
        repo_doc.save(doc)
...
class TestTramitarDocumentoUseCase:
    def test_tramita_com_sucesso(self):
        ...
        dto = TramitarDocumentoInputDTO(documento_id=doc.id, unidade_origem_id="u1",
            unidade_destino_id="u2", tipo=TipoTramitacao.ENVIO, motivo="Envio para analise", autor_id="u1")
        result = uc.execute(dto)
    ...
    def test_tramita_documento_nao_encontrado(self):
        ...
        repo_tram = FakeTramitacaoRepository()
        uc = TramitarDocumentoUseCase(repo_doc, repo_tram)
        dto = TramitarDocumentoInputDTO(documento_id="nope", unidade_origem_id="u1",
            unidade_destino_id="u2", tipo=TipoTramitacao.ENVIO, motivo="Teste", autor_id="u1")
        with pytest.raises(DocumentoNaoEncontradoError):
            uc.execute(dto)
...
class TestDocumentoEntity:
    def test_status_rascunho_inicial(self):
        d = Documento(codigo="D1", numero="1", ano=2026, titulo="T", tipo_documental_id="t1", unidade_autor_id="u1")
        assert d.status == StatusDocumento.RASCUNHO

    def test_assinado_com_hash(self):
        d = Documento(codigo="D1", numero="1", ano=2026, titulo="T", tipo_documental_id="t1",
            unidade_autor_id="u1", hash_integridade="abc")
...
class TestClassificacaoDocumental:
    ...
    def test_filha_com_pai(self):
        c = ClassificacaoDocumental(codigo="C2", nome="F", prazo_retencao=6, classificacao_pai_id="p1")
        assert c.is_root is False

class TestTramitacaoDocumento:
    def test_nova_nao_concluida(self):
        t = TramitacaoDocumento(documento_id="d1", unidade_origem_id="u1", unidade_destino_id="u2", tipo=TipoTramitacao.ENVIO)
        assert t.is_concluida is False

    def test_concluida_com_recebimento(self):
        t = TramitacaoDocumento(documento_id="d1", unidade_origem_id="u1", unidade_destino_id="u2",
            tipo=TipoTramitacao.ENVIO, data_recebimento=datetime.utcnow())
        assert t.is_concluida is True

class TestProcessoDocumento:
    def test_novo_ativo(self):
        p = ProcessoDocumento(numero="P1", ano=2026, tipo_processo_id="t1", titulo="P", unidade_autor_id="u1")
...
class TestRepositorios:
    def test_documento_save_get(self):
        repo = FakeDocumentoRepository()
        d = Documento(codigo="D1", numero="1", ano=2026, titulo="T", tipo_documental_id="t1", unidade_autor_id="u1", created_by="u1")
        saved = repo.save(d)
        assert repo.get_by_id(saved.id) is not None

    def test_find_by_processo(self):
        repo = FakeDocumentoRepository()
        d = Documento(codigo="D1", numero="1", ano=2026, titulo="T", tipo_documental_id="t1", unidade_autor_id="u1", processo_id="p1", created_by="u1")
        repo.save(d)
